...
async def create_notification(
    ...
    await send_notification(notification_doc)
    
    return str(result.inserted_id)

async def send_notification(notification_doc: Dict[str, Any]):
    ...
    if settings.get("push_notifications", True) and not notification_doc["is_push_sent"]:
        await send_push_notification(notification_doc, settings)
    ...
    if priority in [NotificationPriority.CRITICAL, NotificationPriority.HIGH]:
        if settings.get("email_notifications", True) and not notification_doc["is_email_sent"]:
            await send_email_notification(notification_doc)
...
async def send_push_notification(notification_doc: Dict[str, Any], settings: Dict[str, Any]):
...
async def send_email_notification(notification_doc: Dict[str, Any]):
